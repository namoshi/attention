# development form torchvision/0.15
## vit_b_16
...
class Encoder(nn.Module):
    ...
    # layerごとの出力を返すように変更
    def forward(self, input: torch.Tensor, target_layer=None):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")

        # positional embeddingを付与
        # positional_embeddingは特徴量inputと同じサイズ ( バッチサイズ, トークンの数, トークンの次元数 )
        input = input + self.pos_embedding

        x = self.dropout(input)

        # target_layerに指定がない場合は，最終layerまで通した結果のみ返す
        if target_layer is None:
            x = self.layers(x)
            return self.ln(x), None

        # target_layersに指定がある場合は，中間層の特徴量を格納したfeat_dictも返す
            ...
            if target_layer == 'all':
                for i, layer in enumerate(self.layers):
                    x = layer(x)
                    feat_dict[f'encoder_layer_{i+1}'] = x

            # 特定のlayerの特徴量を得たい場合
            elif 'encoder_layer_' in target_layer:
                # 対象layerの層番号をintで取得
                num_layer = int(target_layer.split("_")[-1])

                # もし指定された層番号がvitの用意されている層を超える場合はエラー
                if num_layer > (len(self.layers)):
                    raise ValueError(f'Select target_layer =< encoder_layer_{len(self.layers)}')
                ...
                raise ValueError(f'Incorrect target_layer ... {target_layer}. Select from < None, all, encoder_layer_n >')
            
            return self.ln(x), feat_dict
...
class VisionTransformer(nn.Module):
    ...
    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 入力を，n_h*n_wこのトークンに分ける + 各トークンの次元数は，hidden_dim
        x = self.conv_proj(x)
        ...
        # embedding dimension
        # 軸の並び替え : 結果，(batchsize n, hiddn_dim, num of token)
        x = x.permute(0, 2, 1)

        return x

    def forward(self, x: torch.Tensor, return_intermediate=False, target_layer=None):
        # Reshape and permute the input tensor
        # 入力をトークンに分ける + 各トークンの次元数をhiddn_dimに．出力は，(batchsize n, hiddn_dim, num of token)
        x = self._process_input(x)
        n = x.shape[0]

        # Expand the class token to the full batch
        # クラスtoken
        # クラスtokenは，1つのトークンと同じ次元数 (バッチサイズ, 1, トークンの次元数)
        batch_class_token = self.class_token.expand(n, -1, -1)
        # クラストークンを連結 x : (バッチサイズ,  トークンの数+1, トークンの次元数)
        x = torch.cat([batch_class_token, x], dim=1)
        
        x, feat_dict = self.encoder(x, target_layer=target_layer)
